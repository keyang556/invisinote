...
- **`InvisinoteSettingsPanel`** — a `gui.settingsDialogs.SettingsPanel` for configuring watched folders and file extensions, registered as a category in NVDA's Settings dialog. Registered in `GlobalPlugin.__init__` and deregistered in `terminate`. `script_open_settings` opens that category directly via `gui.mainFrame.popupSettingsDialog`; it ships with no default gesture — users assign one in NVDA's Input Gestures dialog.
